"""
Page 4 – Rumour Network: interactive claim–source graph using networkx + plotly.
"""
import sys, os
...
for c in claims:
    G.add_node(f"claim_{c.id}",
               kind="claim",
               label=f"C{c.id}: {c.text[:40]}…",
               color=STATUS_COLOR.get(c.status, "#9e9e9e"),
               size=12)

for s in sources:
    G.add_node(f"src_{s.id}",
               kind="source",
               label=f"{s.platform}:{s.handle}",
               color="#1565c0",
               size=8)
...
if G.number_of_nodes() > max_nodes:
    st.warning(f"Graph has {G.number_of_nodes()} nodes – showing top {max_nodes} claims by evidence count.")
    top_claim_ids = sorted(
        [c.id for c in claims],
        key=lambda cid: sum(1 for e in evidence if e.claim_id == cid),
        reverse=True,
    )[:100]
    nodes_to_keep = {f"claim_{cid}" for cid in top_claim_ids}
    nodes_to_keep |= {f"src_{s.id}" for s in sources}
...
for node in G.nodes():
    ...
    node_size.append(data.get("size", 8))
